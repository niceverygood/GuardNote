...
ORANGE = RGBColor(0x03, 0xC7, 0x5A)
ORANGE_DARK = RGBColor(0x00, 0xA6, 0x4B)
PEACH = "E9FBF1"
CREAM = "F7F8F7"
GREEN = RGBColor(0x03, 0xC7, 0x5A)
GREEN_FILL = "E9FBF1"
RED = RGBColor(0x00, 0xA6, 0x4B)
RED_FILL = "EDF8F2"
LINE = "E2E7E4"
...
def build():
    ...
    for step in (
        ...
        "AI가 제안한 답변, 근거 명확성, 문서 근거를 확인한 후 필요한 답변만 점검표에 반영합니다.",
    ...
    for step in (
        ...
        "PDF, DOCX, TXT 문서(최대 6MB)를 선택하거나 샘플 동의서를 사용합니다.",
        "분석 범위를 확인하고 AI 답변 생성을 실행합니다.",
        "문항별 AI 답변, 근거 명확성, 문서 근거를 읽습니다.",
    ...
    for item in (
        ...
        "근거 명확성: 해당 문서가 질문의 근거로 충분하다고 판단한 수준",
        "문서 근거: 답변을 제안한 문구 또는 누락된 정보",
        "반영 결과: 선택한 답변이 자가점검 진행률과 위반 건수에 즉시 반영되는지 여부",
    ):
        add_bullet(doc, item, nums, item.split(":")[0] + ":")
    add_callout(doc, "개인정보 보호", "선택한 문서는 암호화된 연결로 Anthropic Claude API에 전송됩니다. GuardNote 서버와 DB에는 문서 원문을 저장하지 않으며, API 키는 서버 비밀환경변수로만 관리합니다.", "green")
